...
页面外壳是 flex：工具栏与标题占固定高度，卡片区自己滚动（`main { flex:1; overflow:auto }`）。
卡片**永远按 340pt 真实尺寸**排版——窗口变小只会多换几行、在卡片区里滚动，
**绝不缩放卡片**（缩过的卡片看不出真实字号与间距，预览就失去意义了）。
窗口够大时一屏看全，不够大时滚动卡片区，工具栏与标题始终在。
