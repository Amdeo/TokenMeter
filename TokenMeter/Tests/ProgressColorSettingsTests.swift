...
/// 进度条颜色设置：能力由卡片样式与供应商卡片 renderer 各自声明，
/// 编辑流程只在两者都支持进度条时提供入口，颜色本身仍是订阅上的同一份数据。
@MainActor
struct ProgressColorSettingsTests {
    ...
    // MARK: - 入口可用性（编辑流程读的就是这一条判定）

    @Test
    func colorSettingsStayHiddenForCardsWithoutProgressMeters() {
        let balance = Quota(
            name: "API 余额", used: 20, limit: 100, resetAt: nil,
            unit: .currency(code: "CNY", scale: 1), kind: .balance
        )
        for style in SubscriptionCardStyle.allCases {
            #expect(!QuotaColorSettings.isAvailable(style: style, providerID: .deepSeek))
            #expect(QuotaColorSettings.targets(style: style, providerID: .deepSeek, quotas: [balance]).isEmpty)
        }
    }
    ...
    @Test
    func quotaColorPageReturnsToTheEditorItCameFrom() {
        ...
        navigation.showQuotaColorSettings()
        #expect(navigation.route == .quotaColors)
        guard case .quotaColors(let draft) = navigation.content(for: [subscription]) else {
            Issue.record("颜色页应携带当前草稿")
            return
        }
        #expect(draft.original?.id == subscription.id)

        // 在颜色页改色后返回：仍回到同一个编辑页，草稿带着未保存的颜色。
    ...
    @Test
    func quotaColorPageReturnsToTheAddFlowDraft() {
        let navigation = makeNavigation()
    ...
    @Test
    func colorRouteWithoutDraftFallsBackToRecovery() {
        let navigation = makeNavigation()
        navigation.route = .quotaColors

        if case .recovery = navigation.content(for: []) {} else {
            Issue.record("没有草稿时颜色页应回落到恢复页")
        }
    }
    ...
    @Test
    func legacyQuotaColorsDecodeForCardsWithoutProgressMeters() throws {
