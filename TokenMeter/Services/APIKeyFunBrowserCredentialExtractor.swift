...
enum APIKeyFunBrowserCredentialError: LocalizedError, Sendable, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .credentialsMissing:
            return "未找到 APIKEY.FUN 登录态，请先登录后重试。"
        case .invalidCredentials:
...
/// APIKEY.FUN 网页登录态的提取入口：从 localStorage 读出
/// auth_token / refresh_token，校验 JWT 并构造凭证。
/// 内嵌 WKWebView 登录与 token 续期共用同一出口（与 CCBus 实现同构）。
enum APIKeyFunBrowserCredentialExtractor {
    /// 读取 APIKEY.FUN 网页 localStorage 中登录态的 JS，返回形如
    ...
    static func credential(from rawValue: String) throws -> KimiBrowserCredential {
        ...
        guard let accessToken = payload.accessToken?.trimmingCharacters(in: .whitespacesAndNewlines),
            ...
            throw APIKeyFunBrowserCredentialError.credentialsMissing
        }
