...
enum SiyuBrowserCredentialError: LocalizedError, Sendable, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .credentialsMissing:
            return "未找到 Siyu API 登录态，请先登录后重试。"
        case .invalidCredentials:
            return "Siyu API 登录态格式无效，请重新登录后重试。"
        case .expired:
            return "Siyu API 登录态已过期，请重新登录后重试。"
        case .refreshFailed(let message):
            return "刷新 Siyu API 登录态失败：\(message)。请稍后重试。"
        }
    }
...
/// Siyu API 网页登录态的提取入口：从 localStorage 读出
/// auth_token / refresh_token，校验 JWT 并构造凭证。
/// 内嵌 WKWebView 登录与 token 续期共用同一出口（与 CCBus 实现同构）。
enum SiyuBrowserCredentialExtractor {
    /// 读取 Siyu API 网页 localStorage 中登录态的 JS，返回形如
    ...
    static func credential(from rawValue: String) throws -> KimiBrowserCredential {
        guard let data = rawValue.data(using: .utf8),
            ...
            throw SiyuBrowserCredentialError.invalidCredentials
        }
        guard let accessToken = payload.accessToken?.trimmingCharacters(in: .whitespacesAndNewlines),
            ...
            throw SiyuBrowserCredentialError.credentialsMissing
        }
