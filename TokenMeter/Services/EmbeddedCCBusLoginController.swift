...
/// 内嵌 WKWebView 的 CCBus（AI 巴士）账号登录窗口：会话由 App 自持，
/// 用户在 CCBus 官方登录页完成登录后，自动提取 localStorage 登录态。
/// 成功返回凭证；用户关窗、超时或任务取消时抛 CancellationError（调用方静默收尾）。
@MainActor
final class EmbeddedCCBusLoginController: NSObject, NSWindowDelegate, BrowserSessionLogining {
    ...
    private func showPanel() {
        ...
        webView.load(URLRequest(url: CCBusSessionRefresher.loginPageURL))
    ...
    /// 仅在 ccbus.top 域内页面尝试提取；未登录（token 缺失/过期）时返回 nil 继续等待。
    private func extractIfReady() async -> KimiBrowserCredential? {
        guard let webView,
              let host = webView.url?.host?.lowercased(),
              host == "ccbus.top" || host.hasSuffix(".ccbus.top") else { return nil }
        guard let raw = try? await webView.evaluateJavaScript(CCBusBrowserCredentialExtractor.extractionJavaScript) as? String,
              !raw.isEmpty else { return nil }
        return try? CCBusBrowserCredentialExtractor.credential(from: raw)
    }
}
